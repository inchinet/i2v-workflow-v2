...
4. **視覺細節**（選填）
   - 鎖定特定外觀細節：「保持髮型衣着, 耳環眼鏡, 化妝, 場景, 日夜, 室外/室內, 除非轉場境」
   - 確保一致性，除非場景轉換發生
...
### 視覺與環境一致性鎖定 (Visual & Location Consistency Lock)
 ...
 1.  **全面鎖定**: 您輸入的「鎖定視覺細節」規則（例如：`保持髮型衣着, 耳環眼鏡, 化妝, 場景, 日夜, 室外/室內, 除非轉場境`）現在會作為**Priority 2 輔助規則**發送給 AI。
 
 2.  **嚴格優先級機制 (Priorities)**:
     *   **Priority 1: 人物容貌 (Face & Identity)** - 嚴格由**參考照片**決定。
...
**PRIORITY 2: CLOTHING, ENVIRONMENT, CONVERSATION (FROM TEXT)**
...
### 服裝不一致問題
- **新的 Priority 2 機制**：系統現在已設定為「場景描述中的服裝描述」優先於「參考照片」。
- 如果你要角色穿粉色裙，只需在場景描述中寫「穿著粉色連身裙」。AI 會保持參考圖的臉，但換上粉色裙。
- 如果不描述服裝，AI 可能會參考「鎖定視覺細節」中的設定，或回到參考圖的風格。
...
### 面部一致性問題（角色種族/外觀改變）
- **Priority 1 機制**：面部特徵嚴格鎖定於用戶上傳的參考照片。
- 無論場景描述怎麼寫（例如「古裝」、「太空服」），人物的**臉孔、種族、五官**都必須與參考照片完全一致。
- 建議：上傳清晰的正面照片，避免過度遮擋。
